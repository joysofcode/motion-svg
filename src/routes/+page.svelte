  ...
  <g>
    ...
    <g>
      ...
      {#each { length: 21 } as _, i}
        {#if i % 5 === 0}
          <line x1={1 * i} y1={0} x2={1 * i} y2={20} stroke={'#f9fafa'} stroke-width="0.06" />

          <line x1={0} y1={1 * i} x2={20} y2={1 * i} stroke={'#f9fafa'} stroke-width="0.06" />
        {/if}
      {/each}
    </g>
    ...
    <g>
      ...
      {#each [0, 5, 10, 15, 20] as number, i}
        <text x={5 * i} y={-1} font-size="0.8" text-anchor="middle" fill="#f9fafa">
          {number}
        </text>
